...
class DocumentListEmailSendController extends Controller
{
        ...
        if ($nextGrades != null) {
            ...
            if (DB::table('visa_files')->where("id", "=", $visa_file_id)->update(['visa_file_grades_id' => $nextGrades])) {
                ...
                DB::table('visa_file_logs')->insert([
                    ...
                    'content' => 'Evrak listesi gönderildi',
                    'created_at' => date('Y-m-d H:i:s'),
                ]);

                $request->session()->flash('mesajSuccess', 'Evrak listesi gönderildi');
                return redirect('/musteri/' . $id . '/vize');
            } else {
                $request->session()->flash('mesajInfo', 'Güncelleme sırasında sorun oluştu');
                return redirect('/musteri/' . $id . '/vize');
            }
        } else {
    ...
    public function index_islem($id, $visa_file_id)
    {
