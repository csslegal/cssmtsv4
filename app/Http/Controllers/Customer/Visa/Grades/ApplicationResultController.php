...
class ApplicationResultController extends Controller
{
    ...
    public function index($id, $visa_file_id, Request $request)
    {
        ...
        return view('customer.visa.grades.application-result')
            ->with([
        ...
        if ($request->input('sonuc') == 1) {
            ...
            if ($nextGrades != null) {
                $dataArray = array(
                    ...
                    'visa_date' => $request->input('vize_tarihi'),
                    'visa_refusal_reason' => '',
                    'visa_refusal_date' => '',
            ...
            if ($request->has('tercume')) {
                ...
                DB::table('visa_file_logs')->insert([
                    ...
                    'user_id' => $request->session()->get('userId'),
                    'subject' => $visaFileGradesName->getName(),
                    'content' => 'Vize Sonucu OLUMSUZ olarak kaydedildi',
                    'created_at' => date('Y-m-d H:i:s'),
                ]);
                $request->session()->flash('mesajSuccess', 'Kayıt başarıyla yapıldı');
                return redirect('/musteri/' . $id . '/vize');
