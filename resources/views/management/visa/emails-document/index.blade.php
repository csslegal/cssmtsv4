...
@section('content')
    <nav aria-label="breadcrumb">
        <ol  id="breadcrumb" class="breadcrumb p-2 ">
            ...
            <li class="breadcrumb-item active" aria-current="page">Evrak Listesi E-mailleri</li>
        </ol>
    </nav>
    <div class="card card-primary mb-3">
        <div class="card-header bg-primary text-white">
            Evrak Listesi E-mailleri
            <a class="float-end text-white" href="/yonetim/vize/evrak-emaili/create">Ekle</a>
        </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="btn-group" role="group" aria-label="Basic example">
                                    ...
                                    <a href="/yonetim/vize/evrak-emaili/{{ $kayit->id }}/edit">
                                        <button data-bs-toggle="tooltip" data-bs-placement="top" title="Düzenle">
                                            <i class="bi bi-pencil-square "></i>
                                        </button>
                                    </a>
                                    <form action="/yonetim/vize/evrak-emaili/{{ $kayit->id }}" method="post">
                                        @method('DELETE')
                                        @csrf
...
@section('js')
    <script>
        function goster(id) {
            ...
            $.ajax({
                type: 'POST',
                url: "/yonetim/ajax/evrak-emaili",
                data: {
                    'id': id,
                    "_token": "{{ csrf_token() }}",
                },
                success: function(data, status, xhr) {
                    $("#icerikYükle").html(data['content']);
                },
