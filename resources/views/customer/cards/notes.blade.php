@if (isset($customerNotes))
    <div class="card card-danger mb-3">
        <div class="card-header bg-danger text-white" id="notes">Notlar</div>
            ...
            <div class="row">
                <div class="col-12">
                    <form action="/musteri/{{ $baseCustomerDetails->id }}/notes" method="post">
                        ...
                        <button type="submit" class="w-100 mb-2 btn btn-dark text-white  confirm1" data-title="Dikkat!"
                        data-content="Müşteri notu kaydedilsin mi?">Kaydet</button>
                    </form>
                </div>
                ...
                <div class="col-12">
                    <h5 class="fw-bold text-danger">Alınan Notları</h5>
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            <button class="btn btn-dark btn-sm"
                                                onclick="contentLoad('not','{{ $customerNote->id }}')"
                                                data-bs-toggle="modal" data-bs-target="#exampleModal">Göster</button>
                                            @if (session('userTypeId') == 1)
                                                <form method="post"
                                                    action="/musteri/{{ $baseCustomerDetails->id }}/notes/{{ $customerNote->id }}">
                                                    {{ csrf_field() }}
                                                    {{ method_field('DELETE') }}
                                                    <button class="btn btn-danger btn-sm confirm2" data-title="Dikkat!"
